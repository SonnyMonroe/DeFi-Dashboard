...
const appId =
  "TYTTa3sOs7CHotZdhHuOGYOmcMnnIuPB5LiWmtiZRWrp86t5iUgsCp9PojqEr5Mk";
...
async function displayTokens(
  ...
  if (chn != "avalanche") {
    nativePrice = await Moralis.Web3API.token.getTokenPrice({
      chain: chn,
      address: nativeAdd,
    });
  ...
  table =
    table +
    `<td><img src="${chnSymbol}.png" class="tokens"/> ${chnSymbol} </td>`;
  ...
  results.forEach((e, i) => {
    ...
    table =
      table +
      `<td><img src="${e.get("symbol")}.png" class="tokens"/> ${e.get(
        "symbol"
      )} </td>`;
